...
class InternalGeneric {
    ...
    private InternalGeneric(GVariable[] variables) {
    ...
    public Class<?>[] getParameterTypes(Method method) {
        return InternalGeneric.getParamterTypes(this, method);
    }
    ...
    private static InternalGeneric buildGeneric(Class<?> genericType, Map<String, Class<?>> map) {
        ...
        return new InternalGeneric(vars);
    }
     ...
     * @return
     */
    static Class<?>[] getParamterTypes(InternalGeneric generic, Method method) {
    ...
    public static Class<?> get(Class<?> type, Class<?> genericType, int index) {
        return parse(type, genericType).getByIndex(index);
    }
    /**
     ...
     * @return
     */
    public static InternalGeneric parse(Class<?> type, Class<?> genericType) {
        if(!genericType.isAssignableFrom(type)) {
            return new InternalGeneric(new GVariable[0]);
        }
    ...
    private static InternalGeneric parseNormal(Class<?> type, Class<?> genericType) {
    ...
    private static InternalGeneric parseLambda(Class<?> type, Class<?> genericType) {
        try {
    ...
    public static Method getLambdaImplMethod(Class<?> lambdaType) {
        try {
            return (Method) getLambdaBasedMethod(lambdaType);
        } catch (Exception e) {
            return null;
        }
    }
